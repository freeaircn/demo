        ...
        <el-tab-pane label="启停记录">
          <el-form ref="new_record_form" :model="newRecord" :rules="newRecordRules" label-position="top" >
            <el-form-item prop="genIdx" label="机组编号">
              <el-select v-model="newRecord.genIdx" placeholder="选择机组编号" @change="requestGenStartLastLog(newRecord.genIdx)" >
...
import { getGenStartLastLog } from '@/api/device'
...
export default {
  ...
  methods: {
    ...
    requestGenStartLastLog(genIdx) {
      // getGenStartLastLog(this.stationIdx, genIdx)
      //   .then(function(data) {
